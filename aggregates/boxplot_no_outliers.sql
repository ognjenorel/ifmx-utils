...
create function boxplot_final(final list(int not null)) returning blob;
   ...
   system "cd /working_server_dir; java8 -cp . BoxPlot " || trim(in_file) || " "|| trim(out_file);
   let b = filetoblob("/working_server_dir/out_" || rnd || ".png", 'server');
   return b;
end function;

drop aggregate boxplot ;
create aggregate boxplot with
   (init = histogram_init,
    iter = histogram_iter,
